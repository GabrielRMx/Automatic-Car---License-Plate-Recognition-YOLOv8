...
def draw_border(img, top_left, bottom_right, color=(0, 255, 0), thickness=3, line_length_x=180, line_length_y=180):
    x1, y1 = top_left
    x2, y2 = bottom_right
...
for car_id in np.unique(results['car_id']):
    ...
    #print('LICENSE_TEXT: ', results[(results['car_id'] == car_id) & (results['license_number_score'])]['license_number'])
    #print(license_plate[car_id]['license_number'])

    cap.set(cv2.CAP_PROP_POS_FRAMES, results[(results['car_id'] == car_id)]['frame_nmr'].loc[0])
    ret, frame = cap.read()

    x1, y1, x2, y2 = ast.literal_eval(results[(results['car_id'] == car_id)]['license_plate_bbox'].iloc[
    ...
    if ret:
        ...
        for row_indx in range(len(df_)):
            ...
            try:
                ...
                cv2.putText(frame,
                            license_plate[df_.iloc[row_indx]['car_id']]['license_plate_number'],
                            (int((car_x2 + car_x1 - 200) / 2), int(car_y1 - H - 100 + (text_height / 2))),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            1.3,
                            (0, 0, 0),
                            4)
